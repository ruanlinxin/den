...
export async function loadConfig(): Promise<Config> {
  ...
  const url = process.env.DEN_URL ?? process.env.STASH_URL ?? rc.url;
  const token = process.env.DEN_TOKEN ?? process.env.STASH_TOKEN ?? rc.token;
  if (!url || !token) {
    console.error(
      '[den] 未配置。运行 `den config set --url <URL> --token <TOKEN>`,或设置 DEN_URL/DEN_TOKEN。',
    );
    process.exit(1);
  }
...
export async function request(
  ...
  const res = await fetch(`${cfg.url}${p}`, { ...init, method, headers });
  return res;
}
...
export async function cmdPush(cfg: Config, args: string[]): Promise<void> {
  ...
  if (msg !== undefined) {
    ...
    if (!file) {
      console.error('[den] 用法: den push <file> 或 den push -m "<文本>"');
      process.exit(1);
    }
...
export async function cmdTag(cfg: Config, args: string[]): Promise<void> {
  ...
  if (!id || (op !== 'add' && op !== 'rm') || !rest) {
    console.error('[den] 用法: den tag <id> add <a,b> | den tag <id> rm <tag>');
    process.exit(1);
  }
  if (op === 'add') {
...
export async function cmdConfig(args: string[]): Promise<void> {
  ...
  if (sub === 'show') {
    ...
    console.log(`token: ${rc.token ?? '(未设置)'}`);
    return;
  }
  if (sub === 'set') {
  ...
  console.error('[den] 用法: den config set --url <u> --token <t> | den config show');
  process.exit(1);
}
...
export async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case 'tag':
      ...
      return;
    default:
      console.error(`[den] 未知命令: ${cmd}`);
      usage();
      process.exit(1);
  }
}

// 仅当作为入口脚本直接运行时启动(jest import 时 JEST_WORKER_ID 已设,不触发)
